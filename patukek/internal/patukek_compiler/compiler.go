...
func New() *Compiler {
	...
	for i, b := range patukek_obj.Builtins {
...
func (c *Compiler) AddConstant(o patukek_obj.Object) int {
...
func (c *Compiler) setLastInstruction(op patukek_code.Opcode, pos int) {
...
func (c *Compiler) Emit(opcode patukek_code.Opcode, operands ...int) int {
	ins := patukek_code.Make(opcode, operands...)
	pos := c.AddInstruction(ins)
	c.setLastInstruction(opcode, pos)
	return pos
}

func (c *Compiler) LastIs(op patukek_code.Opcode) bool {
...
func (c *Compiler) ReplaceOperand(opPos, operand int) {
	op := patukek_code.Opcode(c.scopes[c.scopeIndex].instructions[opPos])
	newInst := patukek_code.Make(op, operand)
	c.replaceInstruction(opPos, newInst)
}

func (c *Compiler) ReplaceLastPopWithReturn() {
	lastPos := c.scopes[c.scopeIndex].lastInst.Position
	c.replaceInstruction(lastPos, patukek_code.Make(patukek_code.OpReturnValue))
	c.scopes[c.scopeIndex].lastInst.Opcode = patukek_code.OpReturnValue
}
...
func (c *Compiler) LeaveScope() (patukek_code.Instructions, []patukek_err.Bookmark) {
	ins := c.scopes[c.scopeIndex].instructions
...
func (c *Compiler) Bookmark(pos int) {
	...
	b := patukek_err.NewBookmark(c.fileContent, pos, c.Pos())
...
func (c *Compiler) UnresolvedError(name string, pos int) error {
	...
	return patukek_err.New(c.fileName, c.fileContent, pos, "undefined variable %s", name)
}
...
func (c *Compiler) LoadSymbol(s Symbol) int {
	switch s.Scope {
	case GlobalScope:
		return c.Emit(patukek_code.OpGetGlobal, s.Index)
	case LocalScope:
		return c.Emit(patukek_code.OpGetLocal, s.Index)
	case BuiltinScope:
		return c.Emit(patukek_code.OpGetBuiltin, s.Index)
	case FreeScope:
		return c.Emit(patukek_code.OpGetFree, s.Index)
	case FunctionScope:
		return c.Emit(patukek_code.OpCurrentClosure)
	default:
		return 0
	}
}